...
- 异常恢复中间件：捕获 panic，输出请求头摘要、错误栈，区分"Broke Pipe"等场景。

**更新** Web 中间件在计算请求耗时时采用了性能优化，使用 `time.Since(start)` 替代了 `time.Now().Sub(start)`，减少了计算开销，在高并发场景下提升了性能表现。
...
- Gin/GORM 集成
  ...
  - **Web 中间件性能优化**：使用 `time.Since(start)` 替代 `time.Now().Sub(start)` 计算耗时，减少计算开销，提升高并发场景性能
- 内存与 GC
  - 避免在日志中拼接大型结构体；必要时延迟求值
  - 控制每条日志字段数量，减少临时对象分配

**章节来源**
- [fast_web/InitWebEx.go:84-85](file://fast_web/InitWebEx.go#L84-L85)
...
**更新** Web 日志中间件的性能优化进一步提升了系统的整体性能表现，特别是在高并发场景下，通过减少时间计算开销来提升响应速度和吞吐量。
...
### Web 中间件性能优化详情
**新增** Web 中间件在计算请求处理耗时方面进行了性能优化：

- **优化前**：使用 `time.Now().Sub(start)` 计算耗时
- **优化后**：使用 `time.Since(start)` 计算耗时
- **性能收益**：减少了不必要的时间获取操作，降低了 CPU 开销
- **适用场景**：特别适用于高并发 Web 服务，能够显著提升请求处理吞吐量
