...
func (d Dir) Open(name string) (File, error) {
	...
	path, err := FromFS(path.Clean("/" + name))
	if err != nil {
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) {
	...
	code := http.StatusOK
	oName := strings.TrimSuffix(name, ".gz")
	// If Content-Type isn't set, use the file's extension to find it, but
	// if the Content-Type is unset explicitly, do not sniff the type.
	ctypes, haveType := w.Header()["Content-Type"]
	var ctype string
	if !haveType {
		ctype = mime.TypeByExtension(filepath.Ext(oName))
		if ctype == "" {
...
func serveFile(w http.ResponseWriter, r *http.Request, fs http.FileSystem, name string, redirect bool) {
	...
	if strings.Contains(r.Header.Get("Accept-Encoding"), "gzip") {
		// 如果客户端支持gzip，优先使用本地已存在的gz文件
		f, err = fs.Open(name + ".gz")
	} else {
		err = errors.New("客户端不支持gzip,跳过，继续找原始文件")
	}
	if err != nil {
		f, err = fs.Open(name)
		if err != nil {
